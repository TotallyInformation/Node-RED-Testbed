...
RED.events.on('ti-testbed:ti-dummy-node-added', (node) => {
    console.log('⚙️ [dummy] ti-testbed:ti-dummy-node-added was fired', node, this)
    tiTestbed.watchNodeBox(node.id, 'click', function(event, target) {
        console.log(`⚙️ [dummy] watchNodeBox:${event.type}:${node.id}`, { event, target, })
        tiTestbed.changeNodeBox(node.id, {})
    })
})
RED.events.on('ti-testbed:ti-dummy-node-changed', (node) => {
    console.log('⚙️ [dummy] ti-testbed:ti-dummy-node-changed was fired', node)
})
RED.events.on('ti-testbed:ti-common-node-deleted', (node) => {
    console.log('⚙️ [dummy] ti-testbed:ti-dummy-node-deleted was fired', node)
})

// Don't seem to work - see dummy customNode.js
RED.events.on('ti-testbed:ti-common-run', (data) => {
    console.log('⚙️ [dummy] on ti-testbed:ti-common-run', data)
})
RED.comms.on('ti-testbed:test', (data) => {
    console.log('⚙️ [dummy] on ti-testbed:test', data)
})
RED.events.on('ti-testbed:test2', (data) => {
    console.log('⚙️ [dummy] on ti-testbed:test2', data)
})
// #endregion ---
...
function validateTopic(value) {
    ...
    if ($('#node-input-topic').is(':visible')) {
        console.log('⚙️ [dummy] #node-input-topic IS VISIBLE. ', `Changed? '${this.changed}'.`, `Value: '${value}'.`, `this.topic: '${this.topic}'.`, `jQ value: ${$('#node-input-topic').val()}.`)
    } else {
        console.log('⚙️ [dummy] #node-input-topic IS NOT VISIBLE. ', `Changed? '${this.changed}'.`, `Value: '${value}'.`, `this.topic: '${this.topic}'.`, `jQ value: ${$('#node-input-topic').val()}.`)
    }
