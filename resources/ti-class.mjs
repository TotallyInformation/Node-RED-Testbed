// @ts-nocheck
/** Now loaded as a module so no need to further Isolate this code */
...
RED.events.on('ti-testbed:ti-dummy-node-added', (node) => {
    ...
    tiTestbed.watchNodeBox(node.id, 'click', function(event, target) {
        console.log(`⚙️ [ti-class] watchNodeBox:${event.type}:${node.id}`, { event, target, })
        tiTestbed.changeNodeBox(node.id, {})
    })
})
RED.events.on('ti-testbed:ti-dummy-node-changed', (node) => {
    console.log('⚙️ [ti-class] ti-testbed:ti-dummy-node-changed was fired', node)
})
RED.events.on('ti-testbed:ti-common-node-deleted', (node) => {
    console.log('⚙️ [ti-class] ti-testbed:ti-dummy-node-deleted was fired', node)
})
// #endregion ---
...
function validateTopic(value) {
    ...
    if ($('#node-input-topic').is(':visible')) {
        console.log('⚙️ [ti-class] #node-input-topic IS VISIBLE. ', `Changed? '${this.changed}'.`, `Value: '${value}'.`, `this.topic: '${this.topic}'.`, `jQ value: ${$('#node-input-topic').val()}.`)
    } else {
        console.log('⚙️ [ti-class] #node-input-topic IS NOT VISIBLE. ', `Changed? '${this.changed}'.`, `Value: '${value}'.`, `this.topic: '${this.topic}'.`, `jQ value: ${$('#node-input-topic').val()}.`)
    }
